...
<template>
    <div class="flex flex-col mt-32">
        ...
        <div class="accordion-example w-full flex items-center justify-center brico-200  text-slate-950">
            <ul class="accordion flex justify-center flex-col w-full items-center">
              <li class="accordion-item py-2 w-3/4" v-for="q in faq" :key="q.id" v-auto-animate>
                <div
                  class="question cursor-pointer flex items-center p-4 text-2xl bg-white rounded-md shadow hover:bg-gray-100 transition"
                  @click="current = current === q.id ? false : q.id"
                >
                  <span class="font-semibold text-lg">Q</span>
                  <span class="ml-2">{{ q.question }}</span>
                </div>
                <p v-if="q.id === current" v-html="q.answer" class="w-full p-4 text-xl text-gray-50" />
              </li>
            </ul>
          </div>
    </div>
